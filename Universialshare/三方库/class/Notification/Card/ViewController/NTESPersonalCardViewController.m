...
@interface NTESPersonalCardViewController ()<NIMUserManagerDelegate,UIAlertViewDelegate>
...
- (void)deleteFriend{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"删除好友" message:@"删除好友后，将同时解除双方的好友关系" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alert show];
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        ...
        [[NIMSDK sharedSDK].userManager deleteFriend:self.userId completion:^(NSError *error) {
            [SVProgressHUD dismiss];
            if (!error) {
                [self.view makeToast:@"删除成功"duration:2.0f position:CSToastPositionCenter];
                [self refresh];
            }else{
