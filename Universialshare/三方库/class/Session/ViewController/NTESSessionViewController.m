...
- (void)enterHistory:(id)sender{
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"选择操作" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"搜索本地消息记录",@"清空本地聊天记录", nil];
    sheet.tag = 10;
...
- (void)forwardMessage:(id)sender
{
    ...
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"选择会话类型" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"个人",@"群组", nil];
    sheet.tag = 12;
    [sheet showInView:self.view];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (actionSheet.tag == 12) {
        ...
        switch (buttonIndex) {
            ...
            case 1:{ //清空聊天记录
                UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"确定清空聊天记录？" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"确定", nil];
                sheet.tag =11;
                [sheet showInView:self.view];
                break;
            }
 ...
 - (void)forwardMessage:(NIMMessage *)message toSession:(NIMSession *)session
{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"确认转发" message:tip delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    [alert show];
    _message = message;
    _session = session;
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if(buttonIndex == 1)
    {
        if (_message.messageType == NIMMessageTypeRobot)
        {
            NIMMessage *forwardMessage = [NTESSessionMsgConverter msgWithText:_message.text];
            [[NIMSDK sharedSDK].chatManager sendMessage:forwardMessage toSession:_session error:nil];
        }
        else
        {
            [[NIMSDK sharedSDK].chatManager forwardMessage:_message toSession:_session error:nil];
        }
