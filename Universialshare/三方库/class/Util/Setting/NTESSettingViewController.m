...
@interface NTESSettingViewController ()<NIMUserManagerDelegate,UIActionSheetDelegate>
...
//    NSString *customNotifyText  = [NSString stringWithFormat:@"自定义系统通知 (%zd)",customNotifyCount];
    ...
    NSArray *data = @[
                      @{
                          ...
                          FooterTitle:@"在iPhone的“设置- 通知中心”功能，找到此应用程序，可以更改新消息提醒设置"
                        },
                       @{
                          HeaderTitle:@"",
                          RowContent :@[
                      ...
                      @{
                          ...
                          RowContent :@[
                                  @{
                                      Title      :@"呢称",
                                      DetailTitle:me.userInfo.nickName.length ? me.userInfo.nickName : @"未设置",                                      CellAction :@"onTouchNickSetting:",
                                      ShowAccessory : @(YES)
                                      },
                                  ],
                          FooterTitle:@""
...
- (void)onTouchCleanAllChatRecord:(id)sender{
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"确定清空所有聊天记录？" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"确定" otherButtonTitles:nil, nil];
