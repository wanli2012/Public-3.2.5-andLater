...
- (void)addTags:(NSArray *)tags tag_ids:(NSArray *)tag_ids  selectArr:(NSArray*)selectArr
{
    for (int i = 0; i< tags.count; i++) {
        [self addLabel:tags[i] tag:[tag_ids[i] integerValue] selectArr:(NSArray*)selectArr];
...
- (void)addLabel:(NSString *)text tag:(NSInteger)tag selectArr:(NSArray*)selectArr{
    ...
    for (int i = 0; i < selectArr.count; i++) {
        if ([selectArr[i] integerValue] == tag) {
            selet = YES;
        }
    }
    QQTagItem *Item = nil;
    if (selet == YES) {
         Item = [[QQTagItem alloc]initWith:QQTagStyleSlect];
    }else{
       Item = [[QQTagItem alloc]initWith:QQTagStyleNone];
    }
