//
//  LBProductAuditSucessViewController.m
//  Universialshare
//
...
@interface LBProductAuditSucessViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate,SUDropMenuDelegate>
...
@property (strong, nonatomic)SUPopupMenu *popupViewone;
@property (assign, nonatomic)NSInteger currentIndex;//被选cell

@end

@implementation LBProductAuditSucessViewController
...
-(void)initdatasource{
    ...
    [NetworkManager requestPOSTWithURLStr:@"Shop/getStoreGoodsList" paramDic:@{@"uid":[UserModel defaultUser].uid , @"token":[UserModel defaultUser].token , @"page" :[NSNumber numberWithInteger:self.page],@"type":@"2"} finish:^(id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.dataarr[indexPath.row][@"sh_status"]integerValue] == 2 && [self.dataarr[indexPath.row][@"status"]integerValue] == 2){
        cell.stuasimage.hidden = NO;
        cell.stuasimage.image = [UIImage imageNamed:@"已下架"];
        cell.productView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    }else{
        cell.stuasimage.hidden = YES;
        cell.productView.backgroundColor = [UIColor whiteColor];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGPoint piont = CGPointMake(SCREEN_WIDTH/2.0, rect.origin.y + 30);
    
    self.currentIndex = indexPath.row;
    
    if ([self.dataarr[indexPath.row][@"sh_status"]integerValue] == 2 && [self.dataarr[indexPath.row][@"status"]integerValue] == 2){
         [self.popupViewone presentWithAnchorPoint: piont];
    }else{
        [self.popupView presentWithAnchorPoint: piont];
    }
...
-(void)dropMenuDidTappedAtIndex:(NSInteger)index{
    ...
    if ([self.dataarr[self.currentIndex][@"sh_status"]integerValue] == 2 && [self.dataarr[self.currentIndex][@"status"]integerValue] == 2){
        switch (index) {
            case 0:
                [self  modifyProduct];//修改商品
                break;
            case 1:
                [self  deleteProduct:1];//删除商品
                break;
            case 2:
                [self  deleteProduct:2];//上架商品
                break;
        ...
        switch (index) {
            ...
            case 1:
                ...
                break;
            case 2:
                [self  deleteProduct:3];//下架商品
                break;
            case 3:
                [self  replyComments];//回复
                break;
...
-(void)replyComments{
    ...
    commentVC.dic = self.dataarr[self.currentIndex];
    [self.navigationController pushViewController:commentVC animated:YES];

}

-(void)deleteProduct:(NSInteger)stuats{

    _loadV=[LoadWaitView addloadview:[UIScreen mainScreen].bounds tagert:self.view];
    [NetworkManager requestPOSTWithURLStr:@"Shop/setStoreGoods" paramDic:@{@"uid":[UserModel defaultUser].uid , @"token":[UserModel defaultUser].token , @"goods_id" :self.dataarr[self.currentIndex][@"goods_id"],@"status" :@(stuats)} finish:^(id responseObject) {
        [_loadV removeloadview];
        ...
        if ([responseObject[@"code"] integerValue]==1) {
            ...
            if (stuats == 3) {//下架
                NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:_dataarr[self.currentIndex]];
                dic[@"status"] = @"2";//表示下架
                [self.dataarr replaceObjectAtIndex:self.currentIndex withObject:dic];
...
-(SUPopupMenu*)popupView{
    ...
    if (!_popupView) {
        _popupView = [[SUPopupMenu alloc] initWithTitles: @[@"修改商品",@"删除商品",@"商品下架",@"评论回复"] icons: nil menuItemSize:CGSizeMake(110, 44)];
...
-(SUPopupMenu*)popupViewone{
    
    if (!_popupViewone) {
        _popupViewone = [[SUPopupMenu alloc] initWithTitles: @[@"修改商品",@"删除商品",@"商品上架"] icons: nil menuItemSize:CGSizeMake(110, 44)];
        _popupViewone.delegate = self;
    }
    
    return _popupViewone;
