//
//  LBFrozenRiceTwoViewController.m
//  Universialshare
//
...
@interface LBFrozenRiceTwoViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    LoadWaitView * _loadV;
...
@property (weak, nonatomic) IBOutlet UILabel *maill;
@end

static NSString *ID = @"LBFrozenRiceTableViewCell";

@implementation LBFrozenRiceTwoViewController
...
-(void)repustDatasource{
    ...
    [NetworkManager requestPOSTWithURLStr:@"User/getFrozenMarkList" paramDic:dic finish:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 1){
            ...
            self.rice.text = [NSString stringWithFormat:@"%@",responseObject[@"rice"]];
            self.maill.text = [NSString stringWithFormat:@"%@",responseObject[@"coupons"]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.mallModel = self.dataArr[indexPath.row];
    return cell;
}
