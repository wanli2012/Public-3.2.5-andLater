...
@interface LBHomeIncomesecondViewController ()<UITableViewDelegate,UITableViewDataSource,HWCalendarDelegate>
...
-(void)initdatasource{
    ...
    dic[@"otype"] = @(2);
    dic[@"page"] = @(_page);
    if ([LBSavaTypeModel defaultUser].shop_id == nil || [LBSavaTypeModel defaultUser].shop_id.length <= 0) {
        dic[@"uid"] = [UserModel defaultUser].uid;
        dic[@"token"] = [UserModel defaultUser].token;
    }else{
         dic[@"shop_id"] = [NSString stringWithFormat:@"%@",[LBSavaTypeModel defaultUser].shop_id];
    }
    ...
    [NetworkManager requestPOSTWithURLStr:@"User/getProfitShopList" paramDic:dic finish:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue]==1) {
            ...
            self.total_money.text = [NSString stringWithFormat:@"%@",responseObject[@"total_money"]];
