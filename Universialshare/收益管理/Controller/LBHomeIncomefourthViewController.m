//
//  LBHomeIncomefourthViewController.m
//  Universialshare
//
...
@interface LBHomeIncomefourthViewController ()<UITableViewDelegate,UITableViewDataSource,HWCalendarDelegate>
...
static NSString *ID = @"GLMember_IncomeCell";

@implementation LBHomeIncomefourthViewController
...
-(void)initdatasource{
    ...
    [NetworkManager requestPOSTWithURLStr:@"User/getAchievement" paramDic:dic finish:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue]==1) {
            ...
            if ([responseObject[@"one"] floatValue] >=10000) {
                self.oneLb.text = [NSString stringWithFormat:@"%.1f万",[responseObject[@"one"] floatValue]/10000];
            }
            if ([responseObject[@"two"] floatValue] >=10000) {
                self.twoLb.text = [NSString stringWithFormat:@"%.1f万",[responseObject[@"two"] floatValue]/10000];
            }
            if ([responseObject[@"three"] floatValue] >=10000) {
                self.threeLb.text = [NSString stringWithFormat:@"%.1f万",[responseObject[@"three"] floatValue]/10000];
            }
            if ([responseObject[@"four"] floatValue] >=10000) {
                self.fourLb.text = [NSString stringWithFormat:@"%.1f万",[responseObject[@"four"] floatValue]/10000];
            }
            ...
            for (NSDictionary *dic in responseObject[@"data"]) {
                GLIncomeBusinessModel *model = [GLIncomeBusinessModel mj_objectWithKeyValues:dic];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    GLMember_IncomeCell *cell = [tableView dequeueReusableCellWithIdentifier:ID forIndexPath:indexPath];
