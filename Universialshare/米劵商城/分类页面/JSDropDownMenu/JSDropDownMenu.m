...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height  andWidth:(CGFloat)width{
    ...
    if (self) {
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, origin.y  + height, SCREEN_WIDTH, SCREEN_HEIGHT)];
        ...
        //add bottom shadow
        _bottomShadow = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-0.5, SCREEN_WIDTH, 0.5)];
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            leftTableView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH*ratio, 0);
        ...
        if (rightTableView) {
            ...
            rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH*(1-ratio), 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (leftTableView) {
                leftTableView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH*ratio, tableViewHeight);
            }
            if (rightTableView) {
                rightTableView.frame = CGRectMake(_origin.x+leftTableView.frame.size.width, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH*(1-ratio), tableViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (collectionView) {
            ...
            collectionView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (collectionView) {
                collectionView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, SCREEN_WIDTH, collectionViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
...
- (void)animateTitle:(CATextLayer *)title show:(BOOL)show complete:(void(^)())complete {
    ...
    CGFloat sizeWidth = (size.width < (self.frame.size.width / _numOfMenu) - 25) ? size.width : self.frame.size.width / _numOfMenu - 25;
    title.bounds = CGRectMake(0, 0, sizeWidth, 20);
    complete();
}
